...
{
     public wordBullet bulletPre; // 弾丸のプレハブ
    ...
    public void RemoveWord(Transform boss)
    {
        ...
        wordBullet bullet = Instantiate(bulletPre, transform.position, Quaternion.identity);
    ...
    private void Update()
    {
        if (wordManger.instance.gameEnd) return; // ゲーム終了時は動作を停止
